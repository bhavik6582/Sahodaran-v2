    ...
    <nav className="navbar fixed top-0 left-0 w-full bg-[var(--background-secondary)] shadow-md z-50 transition-colors duration-300">
      <div className="container mx-auto px-4 py-4">
        <div className="flex items-center w-full">
          ...
          {/* Theme Toggle - Moved to right with 80% offset */}
          <div className="ml-auto mr-8">
            <button 
              onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}
              className="relative p-1 rounded-lg hover:bg-[var(--background)] transition-colors duration-300 flex items-center justify-center"
            >
              <m.svg
                strokeWidth="2.5"
                strokeLinecap="round"
          ...
          {/* Navigation Links */}
          <div className={`
            fixed inset-y-0 right-0 w-1/2 bg-[var(--background)] transform ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'}
            md:relative md:translate-x-0 md:bg-transparent md:w-auto
            transition-transform duration-300 ease-in-out
            flex flex-col md:flex-row items-center justify-center md:justify-between
            md:space-x-6 lg:space-x-10
            pt-16 md:pt-0 px-6 md:px-0
          ...
          `}>
            ...
            <button
              onClick={toggleMenu}
              className="md:hidden absolute top-4 right-4 p-2 text-[var(--text-secondary)] hover:text-[var(--accent)] transition-colors duration-300"
